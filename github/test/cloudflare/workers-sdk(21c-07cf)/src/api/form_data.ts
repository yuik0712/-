import { CfWorkerInit, CfModuleType, CfVariable, CfModule, CfKvNamespace, CfCryptoKey } from './worker.js'
import { CfPreviewMode } from './preview.js'
...
function toBinding(name: string, variables: CfVariable): Record<string, unknown> {
...
export function toFormData(worker: CfWorkerInit, preview?: CfPreviewMode): FormData {
    ...
    const metadata = {
      main_module: singleton ? undefined : name, // main module 지정
      body_part: singleton ? name : undefined, // CommonJS인 경우 body_part로 설정
      bindings
    }
